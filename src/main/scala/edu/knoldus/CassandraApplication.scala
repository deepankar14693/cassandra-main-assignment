...
object CassandraApplication extends App with CassandraSession {
 ...
 private def tableCreation(sessionCreate: Session): Unit = {
  println()
  log.debug()
  log.debug("*******table creation and fetching all values*********")
  log.debug()
  ...
  log.debug(s"consistency level set to quorum>>>>>>>>>>>>>>")
 ...
 private def recordsInsertion(sessionCreated: Session): Unit = {
  log.debug()
  log.debug()
  log.debug("*******now inserting records in table emp*********")
 ...
 private def fetchThroughEmployeeId(sessionCreate: Session): Unit = {
  log.debug()
  log.debug()
  log.debug("*******fetching through particular employee id********")
 ...
 private def salaryRecords(sessionCreated: Session): Unit = {
  log.debug()
  log.debug()
  log.debug("salary greater than 30000")
 ...
 private def chandigarhDetails(sessionCreate: Session): Unit = {
  log.debug()
  log.debug()
  log.debug("all chandigarh residents>>>>>>>>>>>>>")
 ...
 private def deleteChandigarhDetails(sessionCreated: Session): Unit = {
  log.debug()
  log.debug()
  log.debug("chandigarh records deleted>>>>>>>>>>>>>>>")
